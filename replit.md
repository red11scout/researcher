...
- **Imported Assessments Preserve Inputs (`importedFromJson: true`):** When a user uploads a previously-exported BlueAlly assessment JSON, the upload route detects it (presence of `analysis.steps[]` with a Step 5) and the client routes it through `POST /api/import-analysis`. That endpoint persists the JSON **verbatim** — it does NOT call Claude, it does NOT call `postProcessAnalysis`, and it does NOT run any of the portfolio caps, CFO reality check, headline override, or normalization passes. The payload is stamped with `importedFromJson: true`, and `evaluateReportStaleness` short-circuits to `stale=false` whenever it sees that flag, so subsequent loads through `/api/reports/:id` and `/api/analyze/check` (and the admin backfill) also leave the imported data untouched. **If you ever need to add a new normalization pass, gate it on `!analysis.importedFromJson`** — the user contract is "imports are immutable; only augment for view, never overwrite."
