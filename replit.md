...
- **Portfolio Cash-Flow Days Cap:** `applyPortfolioCashflowGuardrail` in `src/calc/formulas.ts` enforces `PORTFOLIO_BOUNDS.cumulativeDaysImprovement.max` (= 30 days) across the SUM of per-UC `daysImprovement`. Per-UC `INPUT_BOUNDS.daysImprovement.max` (= 90) does not catch the case where 10 UCs each book 9 credible days against the same revenue base; without this gate the portfolio double-counts the same DSO/DPO/inventory pool. When the cap binds, every cash-flow value is scaled by `30 / sum(days)`, the prorate term is appended to each `Cash Flow Formula` string (preserving audit reconciliation), and a `PORTFOLIO_CASHFLOW_DAYS_CAP` warning is emitted. Locked in by `tests/output-realism.test.ts` (Constellation-Energy-shaped fixture).
