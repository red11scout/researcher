...
**Value Score normalization (April 2026 patch):** `normalizeValueScores()` in `shared/vrm-v2.ts` uses **winsorized log10 percentile normalization** — the 1↔10 band is anchored to the 10th and 90th percentile of `log10(EV/Friction ratio)` for portfolios of 5+ use cases (linear-interpolated Type-7 percentile). Smaller portfolios (2–4 use cases) keep min/max behavior. This replaces the original min/max-based normalization, which was getting dominated by single high-value outliers and crushing every other use case to ~1 (the "all bubbles on the bottom row" failure mode). The active version is exported as `VALUE_NORMALIZATION_VERSION = "v2"` and stamped into `vrm.valueNormalizationVersion` by the postprocessor. Pre-patch reports are flagged stale via `evaluateReportStaleness` (reason `value-norm-version=missing|v1`) so the next admin "Upgrade all reports" run re-normalizes them automatically — no force-mode required.
