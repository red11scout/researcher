...
export function MatrixScorecard({ data, onRowClick }: MatrixScorecardProps) {
  ...
  const tierRank = (t?: string): number => {
    if (t?.includes('Champions')) return 4;
    if (t?.includes('Quick Win')) return 3;
    if (t?.includes('Strategic')) return 2;
    return 1;
  };
  ...
  const sorted = useMemo(() => {
    return [...data].sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case 'priorityTier': av = tierRank(a.priorityTier); bv = tierRank(b.priorityTier); break;
      }
      ...
      const numCmp = sortDir === 'asc' ? (av as number) - (bv as number) : (bv as number) - (av as number);
      if (numCmp !== 0 || sortKey !== 'priorityTier') return numCmp;
